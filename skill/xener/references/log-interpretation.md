...
| Mean `total ... not in kg` > 30% | `not in kg` | KG coverage gap for this organ | Add the target species itself if it's a model organism, or a closer relative; if close relatives are missing/KG-shallow, also add a more distant but well-covered species (see `workflows/species-selection.md`). If even close *and* distant well-covered species barely move the metric, the gap is the KG's shallow coverage for this organ — ACCEPT the gate failure under the correct organ and document it (`mandatory-rules.md` §11); do NOT change the confirmed organ to force a pass. |
