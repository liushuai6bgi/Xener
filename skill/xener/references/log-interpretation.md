...
1. **Check the decay line first** — `total X% homolos of organ[O] not in kg`. If `X > 30%`, the KG has few/no edges for your `model_species` + `organ`. **First try the SOFT fix: expand `model_species` with a relative that exists in the KG — closest first, or a more distant but well-covered species when close relatives are missing or KG-shallow (`model_species` has no hard phylogenetic boundary). Re-run with the previous outdir as `--baseline`; the baseline-relative gate PASSES the new config if the miss improved or held, even if still above 30%.** Only change `organ` (to a different tissue, or `None`) **if `organ` was an unconfirmed guess** — never to a tissue the sample is not, and never to `None`/`Unknown` purely to lift the filter when the organ is a confirmed property of the data. Changing a confirmed organ to move this signal violates `mandatory-rules.md` §11; the honest result there is a PASS under the correct organ with a documented residual WARN.
...
| Many `KG get_gene2celltype_kg returned an empty matrix` | `returned an empty matrix` | KG has no homolo→celltype edges for your `organ` filter | Verify gene names match species conventions; expand `model_species` with a KG-present relative (closest first, or a more distant well-covered species). Drop/change `organ` ONLY if it was an unconfirmed guess — not when the organ is a confirmed tissue of the sample (see `mandatory-rules.md` §11). |
...
| Mean `total ... not in kg` > 30% | `not in kg` | KG coverage gap for this organ | Add the target species itself if it's a model organism, or a closer relative; if close relatives are missing/KG-shallow, also add a more distant but well-covered species (see `workflows/species-selection.md`). Re-run with the prior outdir as `--baseline`: the gate PASSES the config that improved or held the miss, even if still > 30%. If even close *and* distant well-covered species barely move it, the gap is the KG's shallow coverage for this organ — keep the best-scoring config (it still passes as an improvement/hold), document the residual (`mandatory-rules.md` §11); do NOT change the confirmed organ to force a lower number. |
